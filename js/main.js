...
function toGB(numberMB) {
    return parseFloat(numberMB / 1000).toFixed(2);
}
...
function buildBarChart(data) {
    ...
    for (i = 0; i < data.length; i++) {
        if (data[i].Column0.indexOf("strong") !== -1) {} else {
            usagedate.push(data[i].Column0);
            usagedownload.push(toGB(data[i].Column2));
            usageupload.push(toGB(data[i].Column1));
        }
    }
...
function buildDonutChart(data) {
    ...
    totalusage = toGB(totalusage.replace(/.{1,2}strong>/g, ""));

    usageleft = sliceddata[1].Column1;
    usageleft = toGB(usageleft.replace(/.{1,2}strong>/g, ""));
